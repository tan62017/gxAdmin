        ...
        <el-collapse-item title="" :name="item.id">
          <template #title>
            <div class="coll-title flex items-center">
              <span class="mr-10px font-size-18px fw-600"
                >上传CSV{{ index + 1 }}--{{ item.form.symName }}</span
              >
          ...
          >
            <template #sym="{ data }">
              <el-select
                ...
                @change="
                  (val) => {
                    selChange(val, item.form, data.options);
                  }
                "
                clearable
              >
...
const selChange = (val, item, options) => {
  const foption = options.find((i) => i.value === val);
  item.symName = foption.label;
  console.log((val, item, foption));
};
